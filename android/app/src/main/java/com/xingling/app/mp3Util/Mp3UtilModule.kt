package com.xingling.app.mp3Util
...
class Mp3UtilModule(private val reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    fun getMediaMeta(filePaths: ReadableArray, promise: Promise) {
        ...
        for (i in 0 until filePaths.size()) {
            ...
            try {
                ...
                android.util.Log.e("Mp3Util", "getMediaMeta exception for file $i: ${e.message}")
                metas.pushNull()
            } finally {
                try {
                    mmr?.release()
    ...
    fun getMediaCoverImg(filePath: String, promise: Promise) {
        ...
        try {
            ...
            if (coverImg != null) {
                ...
                if (bitmap != null) {
                    try {
                        ...
                        android.util.Log.e("Mp3Util", "Failed to save cover image: ${e.message}")
                        bitmap.recycle()
                        promise.resolve(null)
                    }
                } else {
            ...
            android.util.Log.e("Mp3Util", "getMediaCoverImg exception: ${e.message}", e)
            promise.resolve(null)
        } finally {
            try {
                mmr?.release()
